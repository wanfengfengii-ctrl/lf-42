<script lang="ts">
	...
	import type { SignalGroup, WeatherCondition } from '$lib/types';
	...
	import { Play, Pause, SkipForward, AlertTriangle, CheckCircle, XCircle, Clock, Trophy, Settings } from 'lucide-svelte';
	...
	let weather = $state<WeatherCondition>('calm');
	...
	let weatherOptions: { value: WeatherCondition; label: string }[] = [
		{ value: 'calm', label: '平静' },
		{ value: 'moderate', label: '微风' },
	...
	function resumeTimer() {
		if (!timerInterval && remainingTime > 0 && !answerSubmitted) {
			startTimer();
		}
	}
				...
				<div class="space-y-3">
					...
					<div class="grid grid-cols-2 gap-2">
						{#each weatherOptions as opt}
							<button
								onclick={() => weather = opt.value}
								class="px-3 py-2 rounded-lg text-sm font-medium transition-all {weather === opt.value ? 'bg-primary-500 text-white' : 'bg-surface-200-700-token hover:bg-surface-300-600-token'}"
							>
								{opt.label}
							</button>
						{/each}
	...
	{:else}
		<div class="bg-surface-100-800-token rounded-xl p-6 shadow-lg">
			...
			{#if $currentTrainingSignal}
				<div class="mb-6 p-8 bg-gradient-to-br from-surface-50-900-token to-surface-100-800-token rounded-xl border border-surface-200-700-token">
					<div class="flex items-center justify-center gap-6 min-h-48">
						{#each $currentTrainingSignal.flags as sf, i (sf.flag.id + i)}
							<div class="flex flex-col items-center gap-2">
								<FlagImage flag={sf.flag} size={120} animated={true} weather={weather} />
								<span class="text-sm font-mono text-surface-600-400-token">{sf.flag.code}</span>
							</div>
						{/each}
					</div>
				</div>
				...
				{#if answerSubmitted && currentResult}
					<div class={`mb-6 p-4 rounded-lg border ${getResultStatusClass()}`}>
						...
						{#if ($showAnswerStore || answerSubmitted)}
							<div class="mt-4 pt-4 border-t border-surface-300-600-token/30">
								<p class="text-sm text-surface-500 mb-1">正确答案：</p>
								<p class="font-medium text-surface-900-100-token">{$currentTrainingSignal.meaning}</p>
								{#if currentResult.isMisjudged && !currentResult.isTimeout}
									<p class="text-sm text-surface-500 mt-2">你的答案：<span class="text-error-500">{selectedAnswer || '（未作答）'}</span></p>
								{/if}
							</div>
						{/if}
					</div>
				...
				<div class="flex items-center justify-between">
					...
					<div class="flex gap-3">
						{#if answerSubmitted && !$showAnswerStore}
							<button
								onclick={revealAnswer}
								class="flex items-center gap-2 px-4 py-2 bg-warning-500 hover:bg-warning-600 text-white rounded-lg transition-colors"
							>
								显示答案
							</button>
						{/if}
						...
						{:else}
							<button
								...
								class="flex items-center gap-2 px-6 py-2 bg-success-500 hover:bg-success-600 text-white rounded-lg transition-colors"
							>
								{questionCount >= maxQuestions ? '查看成绩' : '下一题'}
								<SkipForward class="w-4 h-4" />
							</button>
						{/if}
					</div>
		...
		<div class="bg-surface-100-800-token rounded-xl p-4 shadow-lg">
			<div class="grid grid-cols-4 gap-4 text-center">
				<div>
					...
					<div class="text-xs text-success-500">正确</div>
				</div>
				<div>
					<div class="text-2xl font-bold text-error-500">{$statistics.misjudgedAnswers}</div>
					<div class="text-xs text-error-500">误判</div>
				</div>
				<div>
					<div class="text-2xl font-bold text-warning-500">{$statistics.timeoutAnswers}</div>
					<div class="text-xs text-warning-500">超时</div>
				</div>
				<div>
					<div class="text-2xl font-bold text-primary-500">
						{$statistics.totalQuestions > 0 ? Math.round(($statistics.correctAnswers / $statistics.totalQuestions) * 100) : 0}%
					</div>
					<div class="text-xs text-primary-500">正确率</div>
				</div>
			</div>
		</div>
	{/if}
